        ...
        while current < epochs:
            ...
            for index, row in orders_df.iterrows():
                ...
                if checkdate == 0:
                    ...
                    reward = dr.loc[index][0] * holdings
...
if __name__=="__main__":
     sd=dt.datetime(2008,1,1)
    #  ed=dt.datetime(2009,1,1)
    #  prices = calculate_prices(['JPM'],sd,ed)
    #  orders_df = prices.copy().drop(['JPM'],axis=1).assign(Shares = 0)
    ...
    #  sma = calculate_SMA(['JPM'],sd,ed)
    #  sma = sma.fillna(0)
    #  bbval = checkBBVal(prices,sma,calculate_volatility(['JPM'],sd,ed))
    #  bbval = bbval.fillna(0)
    #  print bbval.shape[0]
    #  bbval_sorted = bbval.sort_values(bbval.columns[0],ascending=True)
    ...
    #         disc_thresholds.append(bbval_sorted.iloc[y*stepsize].values)
    

    #  print disc_thresholds
    #  bb_original = bbval.copy()
    #  count = 0
    #  for i in bbval.itertuples():
    #     for j in disc_thresholds:
    #         past_j = j - 1
    #         if i[1] < j and i[1] >= past_j:
    #             bbval.iloc[count,0] = j
    #     count+=1    
